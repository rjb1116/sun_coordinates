...
While _**C**_<sup>es</sup> is the only vector we need to calculate Theta_from_North and Phi_from_Horizon, it's dependent variables are &theta;<sub>LL</sub>, &phi;<sub>LL</sub>, are &theta;<sub>EO</sub> (&phi;<sub>E</sub> is a constant), which we don't actually input to the model. What we do input are a date, time, and longitude and latitude. 
